...
func notFoundHandler(l *net.TCPConn, m *Message) *Message {
	if m.IsConfirmable() {
		return &Message{
...
func (mux *ServeMux) ServeCOAP(l *net.TCPConn, a *net.TCPAddr, m *Message) *Message {
	h, _ := mux.match(m.PathString())
	if h == nil {
		h, _ = funcHandler(notFoundHandler), ""
	}
	// TODO:  Rewrite path?
	return h.ServeCOAP(l, m)
}
...
func (mux *ServeMux) HandleFunc(pattern string,
	f func(l *net.TCPConn, m *Message) *Message) {
